...
int main(int argc, char** argv) try
{
	...
	const std::string objs_directory = argv[1];
	cout << "Hello" << endl;
	...
	std::vector<std::vector<mmod_rect>> obj_boxes_train, obj_boxes_test;
	...
	load_image_dataset(images_train, obj_boxes_train, objs_directory + "/train_so.xml");
	//load_image_dataset(images_test, obj_boxes_test, objs_directory + "/test_quoc_huy_600.xml");


	cout << "num training images: " << images_train.size() << endl;
	//cout << "num testing images:  " << images_test.size() << endl;

	cout << "num obj training is: " << obj_boxes_train.size() << endl;
	...
	//mmod_options options(obj_boxes_train, 18, 18);

	mmod_options options(obj_boxes_train, 60, 52);
	...
	trainer.be_verbose();
	trainer.set_synchronization_file("mmod_sync_so", std::chrono::minutes(5));
	...
	{
		cropper(64, images_train, obj_boxes_train, mini_batch_samples, mini_batch_labels);
	...
	net.clean();
	serialize("mmod_network_so.dat") << net;
	...
	// training data.
	cout << "training results: " << test_object_detection_function(net, images_train, obj_boxes_train) << endl;
	...
	// testing data.
	//cout << "testing results:  " << test_object_detection_function(net, images_test, obj_boxes_test) << endl;
